function M = multinomialsymmetricfixedentriesfactory(pv, S, P)
...
% and the entries in the matrix P are fixed
%
...
% and it is symmetric; and that the entries of P respect the balance
% relation
...
M.name = @() sprintf(['%dx%d symmetric matrices with nonnegative ' ...
    'entries and fixed right and left eigenvectors and given fixed entries'], n, n);
...
%M.dist = @(X, Y) error(['multinomialsymmetricfixedentriesfactory.dist not ' ...
%    'implemented yet.']);
...
M.rand = @random; 
    function X = random()
        X = abs(randn(n, n)).*S;
        X = 0.5*(X+X'); 
        beta = (diag(pv)*P)*ones(n,1); %
        p = diag(pv)*(pv - beta); %

        %Retraction with double_stoch_general in manopt
        Xr = diag(pv)*X*diag(pv);
        [XXr, u,v]= my_doubly_stochastic_general(Xr,p,p, maxDSiters);

        X = diag(u)*X*diag(v);
        X = 0.5*(X+X'); 
        X = X + diag(pv)*P*(diag(pv.^-1)); %
    end
...
M.retr = @retraction;
    ...
    [YYr, u, v]= my_doubly_stochastic_general(Yr, p, p, maxDSiters);
     Y = diag(u)*Y*diag(v);

     Y = Y.*S;
     Y = 0.5*(Y + Y');
     Y = Y + diag(pv)*P*(diag(pv.^-1)); %
    end
